...
void scan(TwoWire &myport) {
  ...
  for (address = 1; address < 127; address++)
  {
    ...
    else if (error == 4)
    {
      ...
      if (errors)
        strcat(errorAddresses, ", ");

      if (address < 16)
        // Serial.print(F("0"));
          strcat(errorAddresses, "0");
      // Serial.println(address, HEX);
      char hexString[3];
      sprintf(hexString, "%X", address);
      strcat(errorAddresses, hexString);
      errors = true;
    }
  }
